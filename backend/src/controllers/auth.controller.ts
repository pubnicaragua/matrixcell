  ...
  async login(req: Request, res: Response) {
    ...
    try {
      ...
      const { data: usuarioData, error: usuarioError } = await supabase
        .from("profile")
        .select("name, created_at, rol_id, roles(name), store_id, store(name)")
        .eq("auth_id", userId)
        .single()
      ...
      const storeId = usuarioData.store_id
      //const storeName = usuarioData.store ? usuarioData.store.name : null
