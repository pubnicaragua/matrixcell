    ...
    async consolidadoEquifax(req: Request, res: Response) {
        try {
            ...
            for (const row of sheetData) {
                ...
                if (existingClients) {
                    console.log('Desde update cliente');
                    const { data: clientData, error: clientError } = await supabase
                        .from('clients')
                        .update(client)
                        .eq('id', clientId)

                    if (clientError) throw new Error(`Error al insertar operación: ${clientError.message}`);
                }

                if (existingOperations) {
                    console.log('Desde update operaciones');
                    const { error: operationError } = await supabase
                        .from('operations').update(operation).eq('client_id', clientId);
                ...
                if (!existingOperations) {
                    const { error: operationError } = await supabase
                        .from('operations').insert(operation);
                    if (operationError) throw new Error(`Error al insertar operación: ${operationError.message}`);
                }
            }
