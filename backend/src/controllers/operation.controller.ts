...
export const OperationController = {
    async getAllOperations(req: Request, res: Response) {
        try {
            ...
            const operations = await BaseService.getAll<Operation>(tableName,['id', 'operation_number', 'operation_type','operation_date','due_date','amount_due','amount_paid','days_overdue','status','judicial_action','created_at','updated_at'],where);
            res.json(OperationResource.formatOperations(operations));
        } catch (error: any) {
